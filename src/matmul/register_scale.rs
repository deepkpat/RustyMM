...
pub unsafe fn macro_kernel_32_scaled(
    ...
    // Scale our micro-loops to process an 8x8 tile! (i steps by 8 now)
    for i in (0..32).step_by(8) {
        for j in (0..32).step_by(8) {
            // Allocate 8 registers for our 8x8 destination tile of C
            ...
            for kk in 0..blocks_per_dim {
                ...
                for k in 0..32 {
                    ...
                    // Broadcast 8 elements from Matrix A
                    ...
                    // Execute 8 FMAs simultaneously per inner iteration
...
pub fn matmul_register_scale(a: &[f32], b: &[f32], c: &mut [f32], n: usize) {
    let blocks_per_dim = n / 32;
    let mut packed_a = vec![0.0f32; n * n];
    let mut packed_b = vec![0.0f32; n * n];

    // Upfront packing remains identical
